...
import { getVariantTheme } from "./dieVariants";
import type { VariantId } from "./config";
import type { DiceParticlesHandle } from "./DiceParticles";
...
export interface DieProps {
  ...
  particlesRef: React.RefObject<DiceParticlesHandle | null>;
}

export default function Die({ slot, rollId, variant, onSettled, particlesRef }: DieProps) {
  const groupRef = useRef<THREE.Group>(null);
  const theme = useMemo(() => getVariantTheme(variant), [variant]);

  const materials = useMemo(() => theme.buildMaterials(), [theme]);
  ...
  useFrame((state, delta) => {
    ...
    if (r.phase === "airborne") {
      ...
      // Emit trail while airborne (variant may disable this, e.g. classic dice).
      if (p.trailEnabled) {
        r.lastTrailEmit += delta;
        if (r.lastTrailEmit > 0.02) {
          r.lastTrailEmit = 0;
          particlesRef.current?.trail(worldPos, { colorA: p.colorA, colorB: p.colorB, gravity: p.gravity });
        }
      }
      ...
      if (!r.bounce1Done && r.t >= total * 0.5) {
        r.bounce1Done = true;
        particlesRef.current?.burst(new THREE.Vector3(group.position.x, FLOOR_Y, group.position.z), {
          strength: 1 * p.burstStrengthScale,
          colorA: p.colorA,
          colorB: p.colorB,
          gravity: p.gravity,
        });
      ...
      if (!r.bounce2Done && r.t >= total * 0.8) {
        r.bounce2Done = true;
        particlesRef.current?.burst(new THREE.Vector3(group.position.x, FLOOR_Y, group.position.z), {
          strength: 0.55 * p.burstStrengthScale,
          colorA: p.colorA,
          colorB: p.colorB,
          gravity: p.gravity,
        });
    ...
    if (r.phase === "settling") {
      ...
      if (tt >= 1 && !r.settleDone) {
        r.settleDone = true;
        particlesRef.current?.burst(new THREE.Vector3(r.originSlot.x, FLOOR_Y, r.originSlot.z), {
          strength: 0.35 * p.burstStrengthScale,
          colorA: p.colorA,
          colorB: p.colorB,
          gravity: p.gravity,
        });
