...
interface Particle {
  ...
  vel: THREE.Vector3;
  size: number;
  colorMix: number; // 0 = deep ice blue, 1 = white frost
}

export interface FrostParticlesHandle {
  /** Sharp radial ice-shard burst, used on dice impact. */
  burst: (position: THREE.Vector3, strength?: number) => void;
  /** Light continuous mist, used while a die tumbles through the air. */
  trail: (position: THREE.Vector3) => void;
}

const FrostParticles = forwardRef<FrostParticlesHandle>((_props, ref) => {
  ...
  useFrame((_, delta) => {
    ...
    for (let i = 0; i < POOL_SIZE; i++) {
      ...
      p.vel.y -= delta * 0.6; // gentle gravity drag on ice shards
      p.pos.addScaledVector(p.vel, delta);

      const lifeT = p.life / p.maxLife;
      const alpha = 1 - lifeT;
      const deepBlue = new THREE.Color("#1c6fa8");
      const frostWhite = new THREE.Color("#eaf9ff");
      const color = deepBlue.clone().lerp(frostWhite, p.colorMix);
